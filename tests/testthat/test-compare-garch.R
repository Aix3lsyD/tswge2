# test-compare-garch.R
# Tests for compare.garch.tse function and its helper functions
...
test_that(".find_best_indices handles normal data", {
  ...
  expect_equal(best$aic, 2)
  expect_equal(best$bic, 3)
  expect_equal(best$aicc, 2)
  expect_equal(best$wlb1, 2)
  expect_equal(best$wlb2, 3)
  expect_equal(best$wlb3, 3)
  expect_equal(best$nyblom, 2)
  expect_equal(best$signbias, 3)
})
...
test_that(".find_best_indices handles NA values", {
  ...
  expect_equal(best$aic, 3)
  expect_true(is.na(best$bic))
  expect_equal(best$aicc, 2)
  expect_equal(best$wlb1, 2)
  expect_equal(best$wlb2, 3)
  expect_true(is.na(best$wlb3))
  expect_equal(best$nyblom, 2)
  expect_equal(best$signbias, 3)
})
...
test_that("compare.garch.tse respects distribution argument", {
  ...
  expect_false(
    results_norm$comparison$AIC[1] == results_std$comparison$AIC[1]
  )
})
...
test_that("compare.garch.tse works with actual GARCH data", {
  ...
  expect_gt(garch11_row$WLB1, 0.01)
})
